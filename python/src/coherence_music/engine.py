...
class CoherenceMusicEngine:
    ...
    def _weights_for_step(
        self, E: float, T: float, PSI: float, dS: float, Esym: float, Lam: float,
        prev_Esym: float, prev_Lam: float
    ) -> Dict[str, float]:
        c = self.config
        ...
        for i in range(n):
            ...
            if cat == "BASE":
                ...
                if not choices:
                    ...
                    intensity = {
                        "E": E, "T": T, "PSI": PSI, "DELTA_S": dS,
                        "E_SYM": abs(Esym - prev_Esym), "LAMBDA": Lam,
                    }.get(cat, 0.5)
