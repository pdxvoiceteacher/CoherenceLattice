...
Vertical stack => Cmaj9 pitch classes: C–E–G–B–D.
"""
...
def taf_theme_notes(velocity: int = 90, dur_beats: float = 1.0) -> List[Phrase]:
    ...
    for label, names in parts.items():
        notes = [Note(midi_from_name(n), float(dur_beats), int(velocity)) for n in names]
        out.append(Phrase(label=f"TAF_{label}", notes=notes, category="TAF"))
    return out


def taf_cmaj9_pitch_classes() -> List[int]:
    """Unique pitch classes (0..11) for Cmaj9: C,E,G,B,D."""
    pcs = {0, 2, 4, 7, 11}
    return sorted(pcs)