...
def test_strict_public_refuses_verifier_override(tmp_path: Path):
    ...
    # overlay registry: two verifiers so we can attempt override
    ...
    # strict public.deliberation manifest with proof_policy
    ...
    (vote / "prover_manifest.json").write_text(json.dumps({
        ...
        "outputs":{"proof_envelope_path":"x.json"},
    ...
    r = subprocess.run(
        [sys.executable, str(script),
         ...
         "--verifier-id", "test.other.groth16"],
        cwd=str(repo_root),
        env=env,
        capture_output=True,
        text=True
    ...
    assert "Overrides disabled in strict public.deliberation" in (r.stdout + r.stderr)
