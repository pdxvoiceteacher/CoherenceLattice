...
def test_disabled_verifier_rejected(tmp_path: Path, monkeypatch):
    vk = tmp_path / "vk.json"
    vk.write_text('{"vk":"x"}', encoding="utf-8")
    ...
    with pytest.raises(ValueError):
        build_proof_envelope_from_commit_and_reveal(commit, reveal, verifier_id="test.disabled")


def test_enabled_groth16_placeholder_raises_not_implemented(tmp_path: Path, monkeypatch):
    vk = tmp_path / "vk.json"
