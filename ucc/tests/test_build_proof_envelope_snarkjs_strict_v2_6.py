...
def test_strict_public_requires_pm_matches_proof_policy(tmp_path: Path):
    ...
    # Two verifiers; vote_manifest wants A, prover_manifest claims B
    reg = tmp_path / "reg.json"
    reg.write_text(json.dumps({
        "test.A": {
            "kind":"snark","alg":"GROTH16","enabled": True,
            "vk_path": str(vk), "vk_sha256": vk_sha, "pin_required": False,
            "signals_schema_path": str(schema_path), "signals_schema_sha256": schema_sha, "signals_schema_required": True,
            "circuit_id": "vote_proof_envelope_public.v1", "circuit_required": True
        },
        "test.B": {
            "kind":"snark","alg":"GROTH16","enabled": True,
            "vk_path": str(vk), "vk_sha256": vk_sha, "pin_required": False,
    ...
    (vote / "vote_manifest.json").write_text(json.dumps({
        ...
        "proof_policy": {"verifier_id":"test.A", "circuit_id":"vote_proof_envelope_public.v1"}
    }), encoding="utf-8")

    # Prover manifest claims verifier_id test.B (mismatch)
    ...
    r = subprocess.run(
        [sys.executable, str(script),
         "--vote-dir", str(vote),
         "--commit", str(vote/"secret_v03/commits/commit_b.json"),
         "--proof-json", str(vote/"proof.json")],
        cwd=str(repo_root),
        env=env,
        capture_output=True,
        text=True
    ...
    assert "prover_manifest verifier_id/circuit_id must match" in (r.stdout + r.stderr)
